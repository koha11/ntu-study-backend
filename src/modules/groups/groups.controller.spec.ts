...
describe('GroupsController', () => {
  ...
  beforeEach(async () => {
    groupsService = {
      ...
      createMeetEventAndInvite: vi.fn().mockResolvedValue({ meet_link: 'https://meet.google.com/abc' }),
      ...
      createGroupCalendarEventAndInvite: vi.fn().mockResolvedValue({ event_id: 'evt-1' }),
      getCanvaPreview: vi.fn().mockResolvedValue({ thumbnail_url: 'https://example.com/img' }),
    };
  ...
  describe('findUserGroups', () => {
    it('returns groups for the authenticated user', async () => {
      groupsService.findUserGroups.mockResolvedValue([{ id: groupId, name: 'G1' }]);
  ...
  describe('listCalendarEvents', () => {
    it('delegates with groupId, userId, and time range', async () => {
      ...
      await controller.listCalendarEvents(makeReq(userId), groupId, query as never);
      expect(groupsService.listGroupCalendarEvents).toHaveBeenCalledWith(
        groupId, userId, query.time_min, query.time_max,
      );
    });
  });
  ...
  describe('createGroupCalendarEvent', () => {
    it('delegates to service and returns event', async () => {
      const body = { title: 'Study Session', start: '2026-06-01T10:00:00Z', end: '2026-06-01T12:00:00Z' };
      const result = await controller.createGroupCalendarEvent(makeReq(userId), groupId, body as never);
      expect(groupsService.createGroupCalendarEventAndInvite).toHaveBeenCalledWith(groupId, userId, body);
  ...
  describe('getCanvaPreview', () => {
    it('returns canva preview for group member', async () => {
      ...
      expect(groupsService.getCanvaPreview).toHaveBeenCalledWith(groupId, userId);
  ...
  describe('findOne', () => {
    it('returns group details for member', async () => {
      ...
      expect(groupsService.findOneForMember).toHaveBeenCalledWith(groupId, userId);
  ...
  describe('getMembers', () => {
    it('returns members list', async () => {
      groupsService.getMembers.mockResolvedValue([{ user_id: userId, full_name: 'Alice' }]);
  ...
  describe('createMeetEvent', () => {
    it('forwards to service with dto', async () => {
      const dto = { title: 'Sync', start: '2026-06-01T10:00:00Z', end: '2026-06-01T11:00:00Z' };
      await controller.createMeetEvent(makeReq(userId), groupId, dto as never);
      expect(groupsService.createMeetEventAndInvite).toHaveBeenCalledWith(groupId, userId, dto);
    });
  });
  ...
  describe('inviteMember', () => {
    it('delegates invitation to service', async () => {
      ...
      expect(groupsService.inviteMember).toHaveBeenCalledWith(groupId, userId, dto.email);
    });
  });
  ...
  describe('toggleMemberStatus', () => {
    it('calls service with group, caller, and target user ids', async () => {
      ...
      await controller.toggleMemberStatus(makeReq(userId), groupId, targetUserId);
      expect(groupsService.toggleMemberStatus).toHaveBeenCalledWith(groupId, userId, targetUserId);
    });
  });
  ...
  describe('removeMember', () => {
    it('forwards ids to service', async () => {
      ...
      expect(groupsService.removeMember).toHaveBeenCalledWith(groupId, userId, targetUserId);
    });
  });
});
